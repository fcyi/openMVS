...
public:
	...
	typedef TImage<Mask> MaskMap; //  累计视差代价的图像对应mask，image of accumulated disparity cost
	typedef TImage<Disparity> DisparityMap; // 视差图，disparity image type
	typedef TImage<AccumCost> AccumCostMap; // 累计视差代价的图像，image of accumulated disparity cost
	...
	enum : int { numDirs = 4 }; // 代价聚合路径数，2 or 4 directions accumulated per pixel pass
	...
	#if SGM_SIMILARITY == SGM_SIMILARITY_CENSUS
	typedef uint64_t Census; // census 值，used to store Census transform
	typedef TImage<Census> CensusMap; // census图，image of Census transforms
	...
	struct ViewData {
		Image8U3 imageColor; // 彩色图，color image
		ImageGray imageGray; // 灰度图，intensity image
		#if SGM_SIMILARITY == SGM_SIMILARITY_CENSUS
		CensusMap imageCensus; // census图像，image of Census transforms (valid per scale)
		#endif
