...
/**
 * @brief MVS对外接口的头文件，调用libs库中提供的稠密重建、网格重建、网格优化、纹理贴图时必须包含，scene类包含了mvs从depth计算到纹理贴图整个功能的所有函数。
 * 成员变量：相机参数，pose，图像，及重建的中间结果pointcloud mesh。
...
public:
	...
	PointCloud pointcloud; // 点云（开始存储的是读入的稀疏点云，用于稠密重建初始化，从而有助于后续的深度估计以及稠密重建，后续存储的是深度图计算融合后稠密点云，以用于后续的网格重建。point-cloud (sparse or dense), each containing the point position and the views seeing it
	...
	unsigned nMaxThreads; // 最大线程，调试过程中最好设为1，使得只有一个主线程在跑，若设为0，则会使用电脑最多可用的线程数来运行程序。  maximum number of threads used to distribute the work load
...
public:
	...
	/**
	 * @brief 选择用来计算reference image的深度图（基于与相邻帧之间的三角关系、视差构造）的邻域帧neighborViews。主要是依据共视的3d特征点个数和特征点与两个图像的相机中心
     ...
     * @param[in] kQual                 这个方法中，从这个参数开始后的没有注释的参数都是用于判断是否为弱支持的曲面，它们就是一些阈值，没有太多的物理意义，详见相应的公式
     * @param[in] kb 
     * @param[in] kf 
     * @param[in] kRel 
     * @param[in] kAbs 
