...
struct intersection_t {
	...
	cell_handle_t ncell; // 最后一个相交面所在四面体的邻域，cell neighbor to the last intersected facet
	vertex_handle_t v1; // 如果是相交于顶点，v1就是该顶点，如果是相交与边上在v1是边的第一个顶点，vertex for vertex intersection, 1st edge vertex for edge intersection
	vertex_handle_t v2; // 相交于边上时边的第二个顶点，2nd edge vertex for edge intersection
	facet_t facet; //  相交的face，intersected facet
	Type type; // 相交的类型：点 边 面，type of intersection (inside facet, on edge, or vertex)
	...
	const Ray3 ray; // 起始点到结束点的连线，the ray from starting point into the direction of the end point (point -> camera/end-point)
