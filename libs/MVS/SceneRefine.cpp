...
public:
	...
	unsigned nAlternatePair; // 控制如何使用图像对里面图像的作为参考图像，using an image pair alternatively as reference image (0 - both, 1 - alternate, 2 - only left, 3 - only right)
	...
	Mesh::VertexFacesArr& vertexFaces; // 每个顶点包含的faces，for each vertex, the list of faces containing it
