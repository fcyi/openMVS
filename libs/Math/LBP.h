...
protected:
	// 有向边 nodeID1->nodeID2，即消息从nodeID1向nodeID2传递
	struct DirectedEdge {
		NodeID nodeID1;  // 起始节点
		NodeID nodeID2;  // 结束节点
	...
	struct Node {
		...
		std::vector<LabelID> labels;        // 节点的所有可能的标签0,1...L
...
public:
	...
	// 根据节点的标签，设置datacost，并将其存放到对应节点的数据结构中
	inline void SetDataCost(LabelID label, NodeID nodeID, EnergyType cost) {
		...
		if (dataCost < node.dataCost) {  // 根据dataCost选择最优的标签label
			node.label = label;
			node.dataCost = dataCost;
		}
		// 初始化每个进入的边的cost为0。由于一开始没有消息传递，每个节点都在等待消息传递
	...
	/**
	 * @brief lbp 迭代优化关键函数，此处的LBP采用的消息更新算法是Min-Sum-message update
	 * 
	...
	void Optimize(unsigned num_iterations) {
		for (unsigned i = 0; i < num_iterations; ++i) {
			...
			for (int_t edgeID = 0; edgeID < (int_t)edges.size(); ++edgeID) {
				...
				// 边的方向是id1->id2，即id2对应终止节点，id1对应起始节点，注意，此处考虑的是有向边。
				// 获取边所对应的两个节点nodes[edge.nodeID1]、nodes[edge.nodeID2]所对应的所有的标签并分别存放到labels1、labels2中
				...
				for (size_t j = 0; j < labels2.size(); ++j) {
					// 考虑当id2节点对应的标签为label2，其每个相邻节点id1传递给id2的消息、代价、能量是多少
					...
					for (size_t k = 0; k < labels1.size(); ++k) {
						...
						// 计算nodeID1的各个进入边的cost之和，即计算除id2外，其余指向id1并与id1相邻的节点传递给id1的消息、代价、能量的累加和
		...
		for (int_t nodeID = 0; nodeID < (int_t)nodes.size(); ++nodeID) {
			...
			for (size_t j = 0; j < node.labels.size(); ++j) {
				...
				// 将所有进入该节点的边的能量值加和即为当前节点的当前label的能量值，也就是对于每个节点选取某个标签后产生的全局能量、代价、消息值。。这相当于是一种后验信息
	...
	EnergyType Optimize() {
		...
		while (true) {
			...
			// 计算新的代价值
			Optimize(1);  // 进行一次迭代，并对节点对应的label以及dataCost进行更新
			...
			if (++i > 100 || diff == EnergyType(0))
				break;
			#if 1
			// 如果能量反向增加了 nIncreases超过3就退出，反之nTotalIncreases>5也退出。由于构建的图中可能会出现环路导致LBP的消息传播无法收敛，因此需要进行此处的限制以避免陷入死循环
